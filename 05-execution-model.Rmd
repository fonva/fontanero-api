...
Cuando utilizas la función `plumb()` en un archivo, Plumber llama la función `source()` sobre ese archivo, el cual evaluará cualquier código de alto nivel que hayas definido.

```{r, echo=FALSE, results='asis'}
include_r("files/apis/05-01-scope.R")
```

Si llamas a la función `plumb()` sobre este archivo, la variable `counter` se creará y residirá en el ambiente creado por esta API. Sin embargo, el *endpoint* no será evaluado hasta que sea invocado en respuesta a una solicitud entrante. Dado que el *endpoint* utiliza `<<-`, el operador de “asignación doble”, cambia la variable `counter` que fue previamente definida cuando se le aplicó la función `plumb()` al archivo. Esta técnica permite a todos los *endpoints* y filtros compartir alguna de la información definida en el nivel superficial de tu API.

## Ambientes

Por defecto, cuando creas un nuevo enrutador Plumber (que sucede de forma implícita cuando llamas a la función `plumb()` sobre un archivo), se crea un nuevo ambiente específicamente para este enrutador. Es en este ambiente en el que todas las expresiones van a ser evaluadas, y todos los *endopoints* serán invocados.

Esto puede ser muy importante, si consideras instaltar enrutadores uno sobre otro. En ese caso, posiblemente esperes que tengan la capacidad de compartir el estado entre ellos vía el ambiente, pero eso no es una función por defecto. Si estás [creando tus router programáticamente](#uso programático), entonces puedes especificar un ambiente a la hora de inicializar tu enrutador Plumber usando el parámetro `envir`. Este es el ambiente en el que:

 - Si se provee una secuencia de comandos de R será leída por la función `source()`.
 - Todas las expresiones serán evaluados.
 - Todos los *endpoint* y las funciones de filtrado serán ejecutadas.
 
Es importante estar consciente de que todos los sub-enrutadores `subrouter`, por defecto, tendrán su propio ambiente. Si deseas que múltiples enrutadores Plumber compartan un ambiente, necesitarás proveer un único ambiente compartido a la hora de crear los enrutadores.

## Desempeño y Procesamiento de la Solicitud

R es un lenguaje de programación de un único hilo, lo que quiere decir que sólo puede realizar una tarea a la vez. Esto sigue siendo así cuando se entregan APIs utilizando Plumber, así que si tienes un único *endpoint* que dura dos segundos en generar una respuesta, quiere decir que cada vez que el *endpoint* sea consultado, su proceso de R no será capaz de responder a ninguna solicitud entrante adicional durante esos dos segundos.

Las solicitudes HTTP entrantes son atendidas en el orden en el que fueron recibidas, pero si las solicitudes entran a una velocidad mayor que la que pueden ser procesadas por el APIbut if requests are coming in more quickly than they can be processed by the API, se encolan en un registro de solicitudes. Las solució común a este problema es utilizar una de las siguientes opciones, o ambas:
